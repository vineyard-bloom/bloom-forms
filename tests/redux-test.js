...
describe('Bloom Form redux', function() {
  ...
  it('adds fields to the example-form', function() {
    ...
    assert.deepEqual(
      store.getState().forms['example-form'].fields,
      exampleFields
    )
  })
})
...
describe('populates fieldNames after initialization', function() {
  it('adds empty values for each new fieldName', function() {
    ...
    newFields.forEach(field => {
      if (
        store.getState().forms['example-form'] &&
        !store.getState().forms['example-form'][field]
      ) {
  ...
  it('adds values to a couple fields', function() {
    ...
    assert.deepEqual(
      store.getState().forms['example-form'].fields,
      endComparison.fields
    )
    assert.equal(
      store.getState().forms['example-form'].isValid,
      endComparison.isValid
    )
  })
})
...
describe('handles errors', function() {
  it('adds an error to blep', function() {
    store.dispatch(
      addFormError('example-form', 'blep', 'This field can\'t be empty.')
    )
  ...
  it('updates a value and deletes an error from blep', function() {
    store.dispatch(
      updateForm(null, 'example-form', 'blep', 'tiny tongue bleps', 'text')
    )
    store.dispatch(
      deleteFormError('example-form', 'blep', 'This field can\'t be empty.')
    )
...
describe('deals with awaitingCheck', function() {
  it('adds fields that are awaitingCheck', function() {
    assert.deepEqual(store.getState().forms['example-form'].awaitingCheck, [])
